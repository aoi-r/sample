...
## v303
効果コンテキスト全体二重発火ガード。イベントhandler内の高リスク効果入口を同一イベント+同一引数で一度だけに制限。


## v305
有界証明レイヤーと実行時不変条件ガードを追加。`tools/v305_bounded_proof.py` で有限モデルの証明義務を機械検査。


## v306
勇者イレブンLv2「一心同体」でテンション0/1/2時に、かくれんぼう以外のれんけい実効果も発動するよう修正。


## v307
キースドラゴンの占いが発動しない問題を修正。v307 resolverで専用占い経路を最終上書き。


## v308
高リスクカードは専用処理を優先し、専用処理後の汎用テキスト効果再適用を遮断するガードを追加。


## v310
クロウズ必中、位置指定なし場に出すの前列優先、シンクロのヒーローLv追従を修正。


## v311
- 攻撃済みユニットが同一ターンに再攻撃可能へ復元される問題を修正。
- コインの交換所は3枚交換時のみ1ドローするよう修正。


## v312
ぷちメタルの一度のみGET、ブラバニクイーンのシンクロ再適用、しんりゅう召喚時願い事の専用復旧を追加。


## v313
4作成済みデッキの効果総点検・専用復旧ガードを追加。


## v314
4デッキ全カードの効果ルートを明示化し、リスク効果の未分類ゼロ監査を追加。


## v315
「場に出す」は召喚ではないため召喚時効果を止めつつ、基本キーワード・シンクロ・特技ダメージなどの常在/場効果と継続オーラを適用/refreshする監査とガードを追加。


## v316
ぷちメタルの一度のみターン終了GET復旧、シンクロ/同調の決定的差分適用でプチファイター等のステータス暴走を防止。
