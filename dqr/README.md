...
## v94 update
- ソロモードのパネルコマンドが表示だけで動かない問題を修正。
- 原因候補:
  - render後にソロパネル/ボタンDOMが差し替わり、初回bindしたイベントが外れる可能性があった。
  - ターン終了はオンライン対戦用処理のままで、ソロ専用のターン進行になっていなかった。
- 修正:
  - ソロパネルのボタンをdocumentイベント委譲で確実に処理。
  - 相手手札追加/表示/リセット、山札トップ追加、手札追加、盤面配置などを実処理へ接続。
  - ソロ専用ターン終了処理を追加。
  - ソロでは相手ターンをスキップし、即次の自分ターンに戻る。
  - 次ターン開始時にMP回復、1枚ドロー、開始時効果を処理。
- 追加ファイル:
  - `data/solo_mode_fix_v94.json`


## v95 update
- ソロ効果テスト部屋の実機操作修正。
- 修正:
  - 味方/敵盤面への配置を直接盤面へ反映する方式に変更。
  - 建物/ダンジョンの耐久値も設定。
  - 相手手札表示をログだけでなく、効果テストパネル内に表示。
  - ターン終了時に終了時効果→開始時効果→1枚ドロー→手札枚数ログを処理。
  - ソロ開始時のMP10/テンションMAXをやめ、MP1/テンション0開始に戻した。
  - MP10/テンションMAXはボタンで任意設定。
  - デッキ内にヒーローカードがあればソロ開始時にヒーロースキルを自動解放。
  - 効果テストパネルを左側に移動し、iPhone safe-area を考慮。
  - ソロ情報表示を追加。
- 追加ファイル:
  - `data/solo_lab_fix_v95.json`
...
- 追加:
  ...
  - 敵/味方配置は「配置モード」に変更。
  - 配置モード中は置ける空きマスが光る。
  - 光っているマスをタップするとそのマスへ配置。
  - 相手手札を相手HUD/リーダー側にカード画像で連動表示。
- 削減:
  - ソロパネル内の長い相手手札ログ表示を削減。
- 追加ファイル:
  - `data/solo_visual_ui_v96.json`


## v97 update
- ソロ効果テスト部屋の手札・配置・テンションスキルを追加修正。
- 修正:
  - 相手手札画像列をタップすると一覧ポップを表示。
  - 自分手札に不明IDがあっても表示をスキップしない。
  - ドロー時に引いたカード名をログ出力。
  - 配置モード中のマスクリックを document capture で強制取得。
  - 通常盤面クリックより前にソロ配置処理へ渡す。
  - ソロ中は同一ターンにテンションを溜めた後でもテンションスキル発動可能。
...
## v101 update
- ソロ効果テスト部屋の操作を直接配線方式に変更。
- 起動画面表示:
  - `v101 / buildable 1465 / total 1582`
- 修正:
  - ソロパネル各ボタンに毎回 `onclick/ontouchend` を直接設定。
  - 指定カードを自分手札へ追加、1枚ドロー、山札トップ、相手手札を直接 `game` に反映。
  - 敵/味方配置ボタンは最初の空きマスへ直接配置。
  - 通常手札UIを固定表示し、手札0枚でも「手札0枚」と表示。
  - 通常手札カードのクリック/タッチを `selectHandCard` に直結。
  - テンションは1ターン1回まで。
  - テンション3からスキル使用可、使用後テンション0。
  - 戦士系テンションスキルはリーダー攻撃力+2/攻撃可能を保証。
- 追加ファイル:
  - `data/solo_direct_controls_v101.json`
